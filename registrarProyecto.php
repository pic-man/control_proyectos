...
if (isset($Ingresar))
   {
    ...
    if(postBlock($_POST['postID'])) 
	 {if($error==0) 
        ...
        $sql="insert into proyecto(pnf,titulo,trayecto,trimestre,asesor,institucional,lapso,lider,registro)values
            ('$pnf','$titulo','$trayecto','$trimestre','$asesor','$institucional','$lapso','$lider','$fecha')";   
        $consulta=mysqli_query($link,$sql)or die(mysqli_error($link));
        
        $sql2="select max(id)as idn,pnf from proyecto where lider=".$lider." and lapso=".$lapso."";
        $consulta2=mysqli_query($link,$sql2);
        if($row2=mysqli_fetch_array($consulta2)){
          $proyecto=$row2['idn'];
          $sql3="insert into proyus(proyecto,usuario,lapso,pnf,registro)values('$proyecto','$lider','$lapso','$pnf','$fecha')";
          mysqli_query($link,$sql3);
        }  
        header('Location:registrarIntegrantes.php?id='.$proyecto);
       }
      }   
     }?>
<!DOCTYPE html>
...
<?php include_once("menu.php");?>    
  <div class="container text-center">
      <h3 class="mt-5">&nbsp;</h3>
      <h3 class="mt-5">Registro de Proyecto<br>1/4</h3>
     <center> 
     <div class="position-relative m-4">
       <!-- <div class="progress" role="progressbar" aria-label="Progress" aria-valuenow="10" aria-valuemin="0" aria-valuemax="100" style="height: 1px;">
       <div class="progress-bar" style="width: 25%"></div>
       </div>
        <button type="button" title="Datos Basicos" class="position-absolute top-0 start-0 translate-middle btn btn-sm btn-primary rounded-pill" 
      ...
      <form id="formRegistrar" action="registrarProyecto.php" method="post" class="row g-3 border">
